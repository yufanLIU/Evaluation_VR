...
[SupportedMFormats,SupportedRFormats]=ReturnSupportMediaFormats(handles);
...
for i=1:NumOfSubDir
    ...
    for j=1:NumOfFiles
        ...
        switch(lower(ext))
            ...
            case SupportedRFormats
                handles.DirData.RawFileLocations(LastElement+j)=1;
            otherwise
    ...
    if(0==NumOfSelFiles)
        ...
        for i=1:NumOfSelFiles
            ...
            case SupportedMFormats
                handles.DirData.NumMediaFiles=handles.DirData.NumMediaFiles+1;
            case SupportedRFormats
                handles.DirData.RawFileLocations(LastElement+i)=1;
        ...
        for i=1:NumOfUnSelFiles
            ...
            switch(lower(ext))
                ...
                case SupportedMFormats
                    handles.DirData.NumMediaFiles=handles.DirData.NumMediaFiles-1;
                case SupportedRFormats
                    handles.DirData.RawFileLocations(handles.DirData.UnSelected)=[];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Raw stream support callbacks
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% --- Executes on selection change in YUVFormat.
function YUVFormat_Callback(hObject, eventdata, handles)
% hObject    handle to YUVFormat (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = get(hObject,'String') returns YUVFormat contents as cell array
%        contents{get(hObject,'Value')} returns selected item from YUVFormat
...
% --- Executes during object creation, after setting all properties.
function YUVFormat_CreateFcn(hObject, eventdata, handles)
% hObject    handle to YUVFormat (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
if(0~=temp)
    handles.DirData.ImageWidth=temp;
    guidata(handles.SelectSource,handles);
end
...
temp=str2num(get(hObject,'String'));
if(0~=temp)
    handles.DirData.ImageHeight=temp;
    guidata(handles.SelectSource,handles);
end
...
HelpPath=which('UICore.html');

web([HelpPath '#FileBrowser'] ,'-browser');
...
function [SupportedMFormats,SupportedRFormats]=ReturnSupportMediaFormats(handles)
...
NumOfformats=length(tmp);
AvailableIFormats={};
for i=1:NumOfformats
    AvailableIFormats=[AvailableIFormats tmp(i).ext];
end
AvailableIFormats=strcat('.',AvailableIFormats(:) )';

AvailableRFormats={'.yuv','.y'};
AvailableVFormats={'.avi','.y4m'};

if('I'==handles.DirData.MediaType)
    SupportedMFormats=intersect(SupportedFormats,AvailableIFormats);
else
    SupportedMFormats=intersect(SupportedFormats,AvailableVFormats);
end

SupportedRFormats=intersect(SupportedFormats,AvailableRFormats);

AllSupportedFormats={SupportedMFormats{:} SupportedRFormats{:} };
UnknownFormats=setdiff(SupportedFormats,AllSupportedFormats);
        ...
        for i=1:handles.DirData.NumListFiles
            ...
            for j=1:NumOfFilesInList
                ...
                if(2==exist(char(filelist(j)),'file'))
                    [fpath,dummy2,ext]=fileparts(char(filelist(j)));
                    if(~isempty(fpath))
                    ...
                    %Copy to final placeholder
                        switch (lower(ext))
                            ...
                            case SupportedMFormats
                                handles.DirData.NumMediaFiles=handles.DirData.NumMediaFiles+1;
                            case SupportedRFormats
                                handles.DirData.RawFileLocations(end+1)=1;
